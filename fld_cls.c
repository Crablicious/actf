...
static int struct_fld_cls_parse(struct json_object *struct_fldc_jobj,
				const struct actf_metadata *metadata,
                                struct actf_fld_cls *struct_fld_cls,
				struct error *e);
...
const char *actf_fld_cls_members_name_idx(const struct actf_fld_cls *fc,
					  size_t i)
{
...
const struct actf_fld_cls *actf_fld_cls_members_fld_cls_idx(const struct actf_fld_cls *fc,
							    size_t i)
{
    if (fc->type == ACTF_FLD_CLS_STRUCT && i < fc->cls.struct_.n_members) {
...
const struct actf_fld *actf_fld_cls_members_attributes_idx(const struct actf_fld_cls *fc,
							   size_t i)
{
    if (fc->type == ACTF_FLD_CLS_STRUCT && i < fc->cls.struct_.n_members &&
...
const struct actf_fld *actf_fld_cls_members_extensions_idx(const struct actf_fld_cls *fc,
							   size_t i)
{
    if (fc->type == ACTF_FLD_CLS_STRUCT && i < fc->cls.struct_.n_members &&
...
const char *actf_fld_cls_options_name_idx(const struct actf_fld_cls *fc,
					  size_t i)
{
...
const struct actf_fld_cls *actf_fld_cls_options_fld_cls_idx(const struct actf_fld_cls *fc,
							    size_t i)
{
    if (fc->type == ACTF_FLD_CLS_VARIANT && i < fc->cls.variant.n_opts) {
...
const struct actf_rng_set *actf_fld_cls_variant_options_selector_rng_set_idx(const struct actf_fld_cls *fc, size_t i)
{
    if (fc->type == ACTF_FLD_CLS_VARIANT && i < fc->cls.variant.n_opts) {
...
const struct actf_fld *actf_fld_cls_options_attributes_idx(const struct actf_fld_cls *fc,
							   size_t i)
{
    if (fc->type == ACTF_FLD_CLS_VARIANT && i < fc->cls.variant.n_opts &&
		       ...
		       struct actf_fld_cls *fc, struct error *e)
{
	...
	fc->alias = NULL;
	fc->attributes = NULL;
	if ((rc = json_object_key_get_ctfjson(fc_jobj, "attributes", &fc->attributes, e)) < 0 &&
	    rc != ACTF_NOT_FOUND) {
	    eprependf(e, "field-class %s", type_name);
	    return rc;
	}
	fc->extensions = NULL;
	if ((rc = json_object_key_get_ctfjson(fc_jobj, "extensions", &fc->extensions, e)) < 0 &&
	    rc != ACTF_NOT_FOUND) {
	    eprependf(e, "field-class %s", type_name);
	    return rc;
	}
...
static int json_object_get_roles(struct json_object *jobj, enum actf_role *roles_out, struct error *e)
{
			     ...
			     struct int_fld_cls *int_fc, struct error *e)
{
    ...
    if ((rc = json_object_key_get_base(fc_jobj, "preferred-display-base", &pref_display_base, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
	return rc;
    }
				       ...
				       struct actf_fld_cls *fc, struct error *e)
{
	...
	return ACTF_OK;
    default:
	eprintf(e, "length \"%" PRIu64 "\" is not supported",
		fxd_len_float->bit_arr.len);
	return ACTF_UNSUPPORTED_LENGTH;
    }
}
				     ...
				     struct actf_fld_cls *fc, struct error *e)
{
    ...
    if ((rc = json_object_key_get_fld_loc(fc_jobj, "length-field-location", &len_fld_loc, e)) < 0
	&& rc != ACTF_NOT_FOUND) {
	return rc;
    } else if (rc == ACTF_NOT_FOUND) {
...
static int variant_fld_cls_opt_parse(struct json_object *opt_jobj,
				     const struct actf_metadata *metadata,
				     struct variant_fld_cls_opt *opt,
				     struct error *e)
{
    int rc;
    ...
    if ((rc = json_object_key_get_rng_set(opt_jobj, "selector-field-ranges", &sel_fld_rng_set, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
					     ...
					     size_t *n_members, struct error *e)
{
	...
	struct json_object *member_cls_jobj = json_object_array_get_idx(member_clses_jobj, i);
	...
	struct json_object *fc_jobj;
        if (! json_object_object_get_ex(member_cls_jobj, "field-class", &fc_jobj)) {
            eprintf(e, "required key field-class is not available in structure member %s", name);
	    rc = ACTF_MISSING_PROPERTY;
            goto err;
        }
        struct actf_fld_cls fc;
	...
	struct ctfjson *attributes = NULL;
	if ((rc = json_object_key_get_ctfjson(member_cls_jobj, "attributes", &attributes, e)) < 0 &&
	    rc != ACTF_NOT_FOUND) {
	...
	struct ctfjson *extensions = NULL;
	if ((rc = json_object_key_get_ctfjson(member_cls_jobj, "extensions", &extensions, e)) < 0 &&
	    rc != ACTF_NOT_FOUND) {
				...
				struct error *e)
{
    ...
    if ((rc = json_object_key_get_alignment(struct_fc_jobj, "minimum-alignment", &min_align, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
        return rc;
    }
				  ...
				  struct actf_fld_cls *fc, struct error *e)
{
    ...
    if ((rc = json_object_key_get_fld_loc(fc_jobj, "selector-field-location", &sel_fld_loc, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
    ...
    if ((rc = json_object_key_get_rng_set(fc_jobj, "selector-field-ranges", &sel_fld_rng_set, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
				 ...
				 struct actf_fld_cls *fc, struct error *e)
{
	...
	for (size_t k = j + 1; k < n_opts; k++) {
	    ...
	    if (actf_rng_set_intersect_rng_set(&opt_a->sel_fld_rng_set, &opt_b->sel_fld_rng_set)) {
		eprintf(e, "the selector-field-ranges of two variant options intersect");
