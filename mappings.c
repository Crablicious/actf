...
int init_uivalt(struct umappings *maps, struct uival **ivs, size_t *ivs_len, struct rb_tree *ivt)
{
	size_t ivals_cap = 0;
	for (size_t i = 0; i < maps->len; i++) {
		...
		for (size_t j = 0; j < rs->len; j++) {
			...
			if (urng_intersect_urng(&rs->rngs[j], &top)) {
				...
				ivals[ivals_len++] = (struct uival) {
					.lower = top.lower,.upper = top.upper,.value =
					    maps->names[i]
				};
				top = rs->rngs[j];
			}
...
/* Mutates maps (sorts ranges in range sets) */
int init_sivalt(struct smappings *maps, struct sival **ivs, size_t *ivs_len, struct rb_tree *ivt)
{
	size_t ivals_cap = 0;
	for (size_t i = 0; i < maps->len; i++) {
		ivals_cap += maps->rng_sets[i].len;
...
int actf_mappings_init(struct mappings *raw_maps, struct actf_mappings *maps)
{
	...
	if (maps->sign == RNG_SINT) {
		rc = init_sivalt(&raw_maps->d.smaps, &maps->ivals.s, &maps->ivals_len, &maps->ivt);
		if (rc < 0) {
			return rc;
		}
		...
		rc = init_uivalt(&raw_maps->d.umaps, &maps->ivals.u, &maps->ivals_len, &maps->ivt);
		if (rc < 0) {
			return rc;
		}
