...
static int fld_cls_from_json(struct json_object *jobj, struct actf_fld_cls *fc, struct error *e)
{
	...
	case json_type_boolean:
		...
		fc->cls.fxd_len_bool = (struct fxd_len_bool_fld_cls) {
			.bit_arr = {.len = 1,.align = DEFAULT_ALIGNMENT}};
		break;
	case json_type_double:
		fc->type = ACTF_FLD_CLS_FXD_LEN_FLOAT;
		fc->cls.fxd_len_float = (struct fxd_len_float_fld_cls) {
			.bit_arr = {.len = 64,.align = DEFAULT_ALIGNMENT}};
		break;
	case json_type_int:
		...
		fc->cls.fxd_len_int = (struct fxd_len_int_fld_cls) {
			.bit_arr = {.len = 64,.align = DEFAULT_ALIGNMENT },
	...
	case json_type_object:{
		...
		fc->cls.struct_ = (struct struct_fld_cls) {.n_members =
			n_members,.min_align = DEFAULT_ALIGNMENT,
			.member_clses = clses
		};
		break;
	}
	...
	case json_type_string:
		...
		fc->cls.null_term_str = (struct null_term_str_fld_cls) {
			.base = {.enc = ACTF_ENCODING_UTF8}};
		break;
	default:
			 ...
			 struct actf_fld *val, struct error *e)
{
	...
	case json_type_int:
		...
		if (fc->type == ACTF_FLD_CLS_FXD_LEN_SINT || fc->type == ACTF_FLD_CLS_VAR_LEN_SINT) {
	...
	case json_type_object:{
		...
		if (n_members != fc->cls.struct_.n_members) {
			eprintf(e,
				"field-class has %zu members while json object has %zu members",
		...
		json_object_object_foreachC(jobj, iter) {
			...
			struct struct_fld_member_cls *mfc =
				&fc->cls.struct_.member_clses[i];
			if (strcmp(name, mfc->name) != 0) {
				eprintf(e,
					"field-class has key %s in index %zu but JSON object has key %s",
					mfc->name, i, name);
				free(vals);
				return ACTF_INTERNAL;
			}
			if ((rc =
			     fld_from_json(root, m_jobj, &mfc->cls, val, &vals[i],
					   e)) < 0) {
				free(vals);
				return rc;
			}
