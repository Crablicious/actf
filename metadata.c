...
const struct actf_fld_cls_alias *actf_metadata_find_fld_cls_alias(const struct actf_metadata *metadata,
								  const char *name)
{
...
static const char *first_extension_enabled(struct ctfjson *extensions)
{
	...
	const struct actf_fld *extension = actf_fld_struct_fld_idx(&extensions->jval.val, i);
	/* Due to ctfjson representing JSON arrays as structs, we can
	 * not verify that the extension was a JSON object rather than
	 * a JSON array. */
	if (actf_fld_type(extension) == ACTF_FLD_TYPE_STRUCT && actf_fld_struct_len(extension) > 0) {
			      ...
			      struct error *e)
{
	...
	if (unsupported_extension) {
	    eprintf(e, "unsupported extension \"%s\" enabled in preamble, unable to consume data stream",
...
static bool environment_valid(struct ctfjson *environment, struct error *e)
{
	...
	if (type != ACTF_FLD_TYPE_SINT &&
	    type != ACTF_FLD_TYPE_UINT &&
	    type != ACTF_FLD_TYPE_STR) {
	    eprintf(e, "environment is invalid in trace class, only JSON integers and strings are allowed.");
	    return false;
	}
    }
    return true;
			       ...
			       struct actf_trace_cls *tc)
{
    ...
    const char *uid = NULL;
    if ((rc = json_object_key_get_string(tc_jobj, "uid", &uid, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
	return rc;
    }
    ...
    if (json_object_object_get_ex(tc_jobj, "packet-header-field-class",
				  &pkt_hdr_jobj)) {
...
static bool actf_clk_origin_eq(const struct actf_clk_origin *o1,
			       const struct actf_clk_origin *o2)
{
    if (o1->type != o2->type) {
	return false;
    }
				...
				const struct actf_clk_cls *clkc2)
{
    ...
    if ((!! clkc1->name != !! clkc2->name) ||
	(clkc1->name && strcmp(clkc1->name, clkc2->name))) {
	return false;
    }
    if ((!! clkc1->uid != !! clkc2->uid) ||
	(clkc1->uid && strcmp(clkc1->uid, clkc2->uid))) {
	return false;
    }
    return true;
}
				 ...
				 struct actf_clk_origin *origin, struct error *e)
{
	...
	if (type == ACTF_CLK_ORIGIN_TYPE_NONE) {
	    eprintf(e, "clock origin has name \"%s\" but no origin has that name", name);
	    return ACTF_NO_SUCH_ORIGIN;
	}
	origin->type = type;
    } else if (jtype == json_type_object) {
	const char *namespace = NULL;
	if ((rc = json_object_key_get_string(clk_origin_jobj, "namespace", &namespace, e)) < 0 &&
	    rc != ACTF_NOT_FOUND) {
	    return rc;
	}
	const char *name;
				...
				struct actf_event_cls **evcp, struct error *e)
{
    ...
    const char *uid = NULL;
    if ((rc = json_object_key_get_string(evc_jobj, "uid", &uid, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
	goto err;
    }
    ...
    if (json_object_object_get_ex(evc_jobj, "payload-field-class",
				  &payload_jobj)) {
    ...
    if (json_object_object_get_ex(evc_jobj, "specific-context-field-class",
				  &spec_ctx_jobj)) {
				  ...
				  struct error *e)
{
    ...
    if ((rc = json_object_key_get_string(ds_cls_jobj, "default-clock-class-id", &def_clkc_id, e)) < 0 &&
	rc != ACTF_NOT_FOUND) {
	goto err;
    }
    ...
    if (json_object_object_get_ex(ds_cls_jobj, "packet-context-field-class",
				  &pkt_ctx_jobj)) {
    ...
    if (json_object_object_get_ex(ds_cls_jobj, "event-record-common-context-field-class",
        ...
        if ((rc = actf_fld_cls_parse(event_common_ctx_jobj, metadata, &event_common_ctx, e)) < 0) {
			    ...
			    const char *ctx, struct error *e)
{
	...
	if (roles & ACTF_ROLE_DEF_CLK_TSTAMP && ! dsc->def_clkc_id) {
	    eprintf(e, "%s has role \"default-clock-timestamp\" but data-stream-class has no default clock", ctx);
	    return ACTF_NO_DEFAULT_CLOCK;
	}
	if (roles & ACTF_ROLE_PKT_END_DEF_CLK_TSTAMP && ! dsc->def_clkc_id) {
	    eprintf(e, "%s has role \"packet-end-default-clock-timestamp\" but data-stream-class has no default clock", ctx);
	    return ACTF_NO_DEFAULT_CLOCK;
	}
	return ACTF_OK;
    }
					    ...
					    struct actf_fld_cls *fc, struct error *e)
{
	...
	if (roles & ACTF_ROLE_METADATA_STREAM_UUID) {
		...
		eprintf(e, "packet-header-field-class has role \"metadata-stream-uuid\" but preamble has no uuid");
		return ACTF_INVALID_UUID_ROLE;
	    }
	    if (fc->type != ACTF_FLD_CLS_STATIC_LEN_BLOB) {
		eprintf(e, "packet-header-field-class has role \"metadata-stream-uuid\" but it is not a static-length-blob");
		return ACTF_INVALID_UUID_ROLE;
	    }
	    if (fc->cls.static_len_blob.len != 16) {
		eprintf(e, "packet-header-field-class has role \"metadata-stream-uuid\" but its length is not the required 16");
		return ACTF_INVALID_UUID_ROLE;
	    }
	}
	return ACTF_OK;
					 ...
					 bool is_first_member, struct error *e)
{
	...
	for (size_t i = 0; i < fc->cls.struct_.n_members; i++) {
	    struct struct_fld_member_cls *member = &fc->cls.struct_.member_clses[i];
	    bool is_top_level = is_top_lvl_struct && (member->cls.type != ACTF_FLD_CLS_STRUCT);
	    if ((rc = int_verify_pkt_magic_num_role(&member->cls, is_top_level, i == 0, e)) < 0) {
		return rc;
	    }
	}
	return ACTF_OK;
	...
	if (roles & ACTF_ROLE_PKT_MAGIC_NUM && (! is_top_lvl_struct || ! is_first_member)) {
	    eprintf(e, "packet-header-field-class has role \"packet-magic-number\" but it is not its first member");
...
static int frag_parse(struct json_object *frag_jobj, struct actf_metadata *metadata)
{
	...
	if (tc.pkt_hdr.type != ACTF_FLD_CLS_NIL &&
	    tc.pkt_hdr.type != ACTF_FLD_CLS_STRUCT) {
	...
	if ((rc = verify_metadata_stream_uuid_role(&metadata->preamble, &tc.pkt_hdr, e)) < 0) {
	    actf_trace_cls_free(&tc);
	    return rc;
	}
	...
	if (clkc->off_from_origin.cycles >= clkc->freq) {
	    eprintf(e, "clock-class has a cycle offset %" PRIu64 " that is greater than or equal the frequency %" PRIu64 "",
	...
	for (size_t i = 0; i < metadata->clk_clses.len; i++) {
	    if (strcmp((*actf_clk_cls_vec_idx(&metadata->clk_clses, i))->id, clkc->id) == 0) {
	...
	if (dsc->event_hdr.type != ACTF_FLD_CLS_NIL &&
	    ...
	    eprintf(e, "event-record-header-field-class is not a structure field class");
	    actf_dstream_cls_free(dsc);
	    return ACTF_NOT_A_STRUCT;
	}
	...
	if (dsc->event_common_ctx.type != ACTF_FLD_CLS_NIL &&
	    ...
	    eprintf(e, "event-record-common-context-field-class is not a structure field class");
	    actf_dstream_cls_free(dsc);
	    return ACTF_NOT_A_STRUCT;
	}
	if (u64todsc_contains(&metadata->idtodsc, dsc->id)) {
	    eprintf(e, "multiple data stream classes with the same id %" PRIu64 "", dsc->id);
	    actf_dstream_cls_free(dsc);
	    return ACTF_DUPLICATE_ERROR;
	}
	if (dsc->def_clkc_id) {
	    for (size_t i = 0; i < metadata->clk_clses.len; i++) {
		struct actf_clk_cls **clkcp = actf_clk_cls_vec_idx(&metadata->clk_clses, i);
	    ...
	    if (! dsc->def_clkc) {
		eprintf(e, "data-stream-class (id %" PRIu64 ") refers to clock-class %s which does not exist",
	...
	if (rc < 0) {
	    eprintf(e, "unable to insert data stream class id %"PRIu64" into hash map", dsc->id);
	    actf_dstream_cls_free(dsc);
	    return ACTF_ERROR;
	}
	...
	if (! dscp) {
	    eprintf(e, "event-record-class (id %" PRIu64 ") refers to data-stream-class id %" PRIu64 " which does not exist",
		    evc->id, evc->dsc_id);
	    actf_event_cls_free(evc);
	    return ACTF_NO_SUCH_ID;
	}
	...
	if (u64toevc_contains(&(*dscp)->idtoevc, evc->id)) {
	    eprintf(e, "multiple event record classes with the same id %" PRIu64, evc->id);
	    actf_event_cls_free(evc);
	    return ACTF_DUPLICATE_ERROR;
	}
	...
	if (rc < 0) {
	    eprintf(e, "unable to insert event class id %"PRIu64" into hash map", evc->id);
	    actf_event_cls_free(evc);
	    return ACTF_ERROR;
	}
    } else {
				 ...
				 struct error *e)
{
	...
	eprintf(e, "metadata packet header size is incorrect; is %#"
		PRIx32"; must be %#"PRIx32, hdr->hdr_sz_bits,
		METADATA_PKT_HDR_SZ_BITS);
	return ACTF_INVALID_METADATA_PKT;
    }
...
const struct actf_fld_cls_alias *actf_metadata_fld_cls_aliases_idx(const struct actf_metadata *metadata, size_t i)
{
...
const struct actf_clk_cls *actf_metadata_clk_clses_idx(const struct actf_metadata *metadata, size_t i)
{
...
const struct actf_dstream_cls *actf_metadata_dstream_clses_next(const struct actf_metadata *metadata,
								struct actf_it *it)
{
