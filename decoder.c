				   ...
				   const struct actf_fld_cls *gen_cls, struct actf_fld *val)
{
	...
	if (sel_val->type == ACTF_FLD_TYPE_BOOL) {
		...
		eprintf(e,
			"selector field of optional field is not a boolean, but there are no selector-field-ranges specified");
				 ...
				 const struct actf_fld_cls *gen_cls, struct actf_fld *val)
{
	...
	if ((rc =
	     do_align(&dec->br, actf_fld_cls_get_align_req(gen_cls), &dec->dec_s.pkt_s, e)) < 0) {
		return rc;
	}
...
static int actf_decoder_pkt_hdrctx_decode(struct actf_decoder *dec)
{
	...
	if (dscp) {
		...
		eprintf(e, "no data stream class with id %" PRIu64 " found", dec_s->pkt_s.dsc.id);
		return ACTF_NO_SUCH_ID;
	}
	...
	if (dec_s->pkt_s.dsc.cls->pkt_ctx.type != ACTF_FLD_CLS_NIL) {
		...
		if (dec_s->pkt_s.opt_flags & PKT_END_DEF_CLK_VAL &&
			...
			eprintf(e,
				"packet beginning timestamp (%" PRIu64
				") is greater than packet end timestamp (%" PRIu64 ")",
				dec_s->pkt_s.def_clk_val, dec_s->pkt_s.end_def_clk_val);
		...
		if (dec_s->pkt_s.content_len > dec_s->pkt_s.tot_len) {
			eprintf(e,
				"packet content length (%" PRIu64
				") larger than total length of packet (%" PRIu64 ")",
				dec_s->pkt_s.content_len, dec_s->pkt_s.tot_len);
				       ...
				       const struct actf_metadata *metadata)
{
	...
	dec->dec_s.ev_arena = (struct arena) {.default_cap =
		    16 * sizeof(struct actf_fld) * evs_cap };
	return dec;
}
...
int actf_decoder_seek_ns_from_origin(actf_decoder *dec, int64_t tstamp)
{
	...
	while (breader_has_bits_remaining(&dec->br)) {
		...
		if (clkc && (pkt_s->opt_flags & PKT_END_DEF_CLK_VAL &&
			     actf_clk_cls_cc_to_ns_from_origin(clkc,
							       pkt_s->end_def_clk_val) < tstamp)) {
