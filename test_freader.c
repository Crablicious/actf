...
void philo_nok_seek_test(struct actf_event_generator gen)
{
	...
	tot_evs = 0;
	CU_ASSERT_EQUAL_FATAL(gen.seek_ns_from_origin(gen.self, INT64_C(1750742598927896798)),
			      ACTF_OK);
	while ((rc = gen.generate(gen.self, &evs, &evs_len)) == 0 && evs_len) {
		tot_evs += evs_len;
	}
	...
	tot_evs = 0;
	CU_ASSERT_EQUAL_FATAL(gen.seek_ns_from_origin(gen.self, INT64_C(1750742599127915322)),
			      ACTF_OK);
	...
	tot_evs = 0;
	CU_ASSERT_EQUAL_FATAL(gen.seek_ns_from_origin(gen.self, INT64_C(1750742599528026369)),
			      ACTF_OK);
	while ((rc = gen.generate(gen.self, &evs, &evs_len)) == 0 && evs_len) {
		tot_evs += evs_len;
	}
	...
	CU_ASSERT_EQUAL_FATAL(gen.seek_ns_from_origin(gen.self, INT64_C(1750802783000000000)),
			      ACTF_OK);
