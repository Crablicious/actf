...
__maybe_unused static bool urng_intersect_srng(const struct urng *rng_a, const struct srng *rng_b)
{
	if (rng_b->upper < 0) {
		return false;
	}
...
bool actf_rng_set_intersect_rng_set(const struct actf_rng_set *restrict rs_a,
				    const struct actf_rng_set *restrict rs_b)
{
