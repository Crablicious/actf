...
{
    eprintf(e, "%s is not a JSON %s", key,
	    json_type_to_str(expected_type));
}
		     ...
		     struct rng *rng_out, struct error *e)
{
    ...
    if (n_eles != 2) {
	eprintf(e, "integer range should be composed of two elements but has %zu elements", n_eles);
	...
	eprintf(e, "lower bound of integer range is smaller than upper bound: [%" PRIi64 ", %" PRIi64 "]", rng.d.s.lower, rng.d.s.upper);
	return ACTF_INVALID_RANGE;
    } else if (rng.type == RNG_UINT && rng.d.u.upper < rng.d.u.lower) {
	eprintf(e, "lower bound of integer range is smaller than upper bound: [%" PRIu64 ", %" PRIu64 "]", rng.d.u.lower, rng.d.u.upper);
	return ACTF_INVALID_RANGE;
    }
...
static int rngs_to_rng_set(struct rng *rngs, size_t n_rngs,
			   enum rng_type sign, struct actf_rng_set *rs,
			   struct error *e)
{
    rs->sign = sign;
    if (rs->sign == RNG_SINT) {
			      ...
			      struct actf_fld_loc *fld_loc, struct error *e)
{
	...
	if (origin == ACTF_FLD_LOC_ORIGIN_NONE) {
	    eprintf(e, "\"origin\" specified in field location but \"%s\" is not a known origin", origin_name);
	    return ACTF_INVALID_FLD_LOC;
	}
    } else {
