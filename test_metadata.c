...
static void test_metadata_ds_cls_attributes(void)
{
	...
	struct actf_it it = { 0 };
	const struct actf_fld *attrs =
	    actf_dstream_cls_attributes(actf_metadata_dstream_clses_next(metadata, &it));
	CU_ASSERT_PTR_NOT_NULL_FATAL(attrs);
	CU_ASSERT(actf_fld_struct_len(attrs) == 2);
	actf_metadata_free(metadata);
...
static void test_metadata_event_cls_with_extensions(void)
{
	...
	const struct actf_event_cls *evc =
	    actf_dstream_cls_event_clses_next(actf_metadata_dstream_clses_next(metadata, &dscit),
					      &evcit);
