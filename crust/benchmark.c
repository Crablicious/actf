...
static int benchmark_urng(struct urng *rngs, size_t rngs_len, uint64_t *queries, size_t n_queries)
{
...
static void print_measurements(const char *victim, int total_intersects)
{
	...
	for (size_t i = 0; i < sizeof(measurements) / sizeof(*measurements); i++) {
		double used_time =
		    ((double) (measurements[i].end - measurements[i].begin)) / CLOCKS_PER_SEC;
